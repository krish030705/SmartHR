...
export default function EmployeeAttendanceList() {
  ...
  useEffect(() => {
    fetchMyAttendance()
      .then(setRecords)
      .catch((err) => setError(err.message || 'Unable to load your attendance.'))
      .finally(() => setLoading(false))
  }, [])

  return (
    <DashboardLayout role="employee" title="My Attendance">
      <div className="rounded-xl border border-black/5 bg-white shadow-card">
        ...
        {!loading && !error && records.length === 0 && (
          <div className="p-6">
            <EmptyState title="No attendance records yet" description="Your check-in history will show up here." />
          </div>
        )}
